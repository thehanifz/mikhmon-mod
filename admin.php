...
include_once('./lib/routeros_api.class.php');  // harus sebelum readcfg (decrypt() ada di sini)
...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  if (isset($_POST['login'])) {
    ...
    if (!rateLimitCheck($clientIP)) {
      ...
      $error = '<div style="width:100%;padding:5px 0;border-radius:5px;" class="bg-danger">
        <i class="fa fa-ban"></i> Terlalu banyak percobaan gagal.<br>
        Coba lagi dalam ' . $remaining_time . ' menit.</div>';
    }
    // ── CSRF validation ───────────────────────────────
    elseif (!csrfValidate()) {
      $error = '<div style="width:100%;padding:5px 0;border-radius:5px;" class="bg-danger">
        <i class="fa fa-ban"></i> Request tidak valid. Silakan coba lagi.</div>';
    }
    else {
      $user = sanitizeInput($_POST['user'], 'username');
      $pass = $_POST['pass']; // password tidak di-sanitize (bisa berisi karakter apapun)

      if ($user == $useradm && mikhmonVerifyPassword($pass, $passadm)) {
        // Login sukses — reset rate limit & regenerate session
        rateLimitReset($clientIP);
        session_regenerate_id(true);
        $_SESSION["mikhmon"] = $user;
        echo "<script>window.location='./admin.php?id=sessions'</script>";
      } else {
        // Login gagal — catat untuk rate limiting
        rateLimitRecord($clientIP);
        $remaining = rateLimitRemaining($clientIP);
        $error = '<div style="width:100%;padding:5px 0;border-radius:5px;" class="bg-danger">
          <i class="fa fa-ban"></i> Username atau password salah.
          ' . ($remaining <= 2 ? '<br>Sisa percobaan: <strong>' . $remaining . '</strong>' : '') . '
        </div>';
      }
    }
  }
