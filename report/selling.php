...
if (!isset($_SESSION["mikhmon"])) {
	...
	// Helper display bulan: "March 2026" atau "Jun 2025"
	...
	$idbl2_display = $idbl2 ? fmtIdbl($idbl2, $_msht, $_mfull) : '';
	if ($idhr != ""){
	...
	if (strlen($idhr) > "0") {
		...
		// Server-side filter ~owner per pattern (v6 + v7), deduplicate by .id
		...
		foreach ($_patterns as $_pat) {
			...
			foreach ($_res as $_row) {
				$_exists = false;
				foreach ($getData as $_ex) {
					if (($_ex['.id'] ?? '') === ($_row['.id'] ?? '')) { $_exists = true; break; }
				}
				if (!$_exists) $getData[] = $_row;
			}
		}
		$TotalReg = count($getData);
		$filedownload = $idbl;
...
<div class="card-header">
	<h3><i class=" fa fa-money"></i> <?= $_selling_report ?> <?= $idhr ? $idhr : $idbl_display; if ($prefix != "") {echo " prefix [" . $prefix . "]";} ?> <small id="loader" style="display: none;" ><i><i class='fa fa-circle-o-notch fa-spin'></i> <?= $_processing ?> </i></small></h3>
</div>
<div class="card-body">
<div class="row">
	<div class="row">
	<div class="col-12">
		<div style="padding-bottom: 5px; padding-top: 5px;">
		  ...
		  <button class="btn bg-primary" onclick="exportTableToCSV('report-mikhmon-<?= $filedownload . $fprefix; ?>.csv')" title="Download selling report"><i class="fa fa-download"></i> CSV</button>
			...
			<?php if(!empty($idbl)){echo '<button name="resume" id="openResume" class="btn bg-primary"title="Resume Report"><i class="fa fa-area-chart"></i> '.$_resume.'</button>';}else{
				echo '<a class="btn bg-primary" href="./?report=selling&idbl='.$idbl2.'&session='.$session.'" title="Show '.$idbl2_display.'" ><i class="fa fa-search"></i> '.$idbl2_display.'</a>';}?>
		...
		<div class="input-group mr-b-10">
			...
			<select style="padding:5px;" class="group-item group-item-md" title="Month" id="M">
        			<?php
										// Array bulan: value=angka, label=nama
										...
										// Ambil bulan aktif sebagai angka
			...
			<script type="text/javascript">
				...
				function filterR(){
					...
					// M = "03", Y = "2026" → idhr=2026-03-02, idbl=2026-03
					if(D !== ""){
						window.location='./?report=selling&idhr='+Y+'-'+M+'-'+D+'&prefix='+X+'&session=<?= $session; ?>';
					}else{
						window.location='./?report=selling&idbl='+Y+'-'+M+'&prefix='+X+'&session=<?= $session; ?>';
					}
