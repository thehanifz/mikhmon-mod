...
class RouterosAPI
{
    ...
    public function connect($ip, $login, $password)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            ...
            // RouterOS v7 improved SSL support
            $ssl_options = array(
                'ssl' => array(
                    'ciphers' => 'ADH:ALL',
                    'verify_peer' => false,
            ...
            if ($this->socket) {
                ...
                if (isset($RESPONSE[0])) {
                    if ($RESPONSE[0] == '!done') {
                        if (!isset($RESPONSE[1])) {
                            // Login method post-v6.43 (including v7)
                            $this->connected = true;
                            $this->detectRouterOSVersion();
                            break;
                        } else {
     ...
     * Stores result in $this->ros_version
     *
     * @return int RouterOS major version
     */
    public function detectRouterOSVersion()
    {
        if ($this->connected) {
            $response = $this->comm('/system/resource/print');
            if (isset($response[0]['version'])) {
                $version = $response[0]['version'];
                $this->ros_version = (int)explode('.', $version)[0];
                $this->debug('RouterOS version detected: ' . $version . ' (major: ' . $this->ros_version . ')');
                return $this->ros_version;
            }
        }
        return 6; // Default to v6 if detection fails
    }
     ...
     * @return int RouterOS major version
     */
    public function getRouterOSVersion()
    {
        return $this->ros_version;
    }
     ...
     * @return boolean
     */
    public function isRouterOSv7()
    {
        return $this->ros_version >= 7;
    }

    /**
     * Get API path based on RouterOS version
     * For paths that changed between v6 and v7
     *
     * @param string $path Base path (e.g., '/system/script')
     * @return string Version-appropriate path
     */
    public function getCompatPath($path)
    {
        // RouterOS v7.20.8 still uses traditional paths
        // Only v7.18+ beta introduced /system/scripting
        // For now, return as-is for compatibility
        return $path;
    }
...
// ─────────────────────────────────────────────────────────
// Encrypt / Decrypt — AES-256-GCM via security.php
// Fungsi encrypt() dan decrypt() di sini adalah wrapper
// agar kompatibel dengan kode lama yang memanggil fungsi ini.
// Enkripsi sesungguhnya ada di include/security.php
// ─────────────────────────────────────────────────────────

function encrypt($string, $key = null) {
    // Gunakan AES-256-GCM dari security.php
    if (function_exists('mikhmonEncrypt')) {
        return mikhmonEncrypt((string)$string);
    }
    // Fallback jika security.php belum di-include (tidak seharusnya terjadi)
    error_log('[MIKHMON] Warning: mikhmonEncrypt() tidak ditemukan, security.php belum di-include.');
    return base64_encode($string);
}

function decrypt($string, $key = null) {
    // Coba AES-256-GCM dulu
    if (function_exists('mikhmonDecrypt')) {
        $result = mikhmonDecrypt((string)$string);
        // Jika hasil kosong, coba legacy decrypt (untuk data lama)
        if ($result === '' && $string !== '') {
            if (function_exists('legacyDecrypt')) {
                return legacyDecrypt((string)$string);
            }
        }
        return $result;
    }
    // Fallback legacy XOR (tidak seharusnya terjadi setelah migrasi)
    error_log('[MIKHMON] Warning: mikhmonDecrypt() tidak ditemukan, security.php belum di-include.');
